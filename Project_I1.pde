...
void setup(){
  ...
  bumpImage = loadImage("bump.jpg"); // Replace "bump_image.jpg" with the actual path to your bump image
  bumpImage2 = loadImage("bump.jpg"); // Replace "bump_image.jpg" with the actual path to your bump image
  pyramids = loadImage("pyramids.JPG");
  edges = loadImage("edges.png");
...
void draw(){
  ...
  if(ffNum == FFNUM.FOUR) {
   ...
   //from an iteration with lots of bumps
    //image(bumpImage, mapX(center2.x), mapY(center2.y), scaledWidth, scaledHeight);
    //image(bumpImage, mapX(center3.x), mapY(center3.y), scaledWidth, scaledHeight);
    //image(bumpImage, mapX(center4.x), mapY(center4.y), scaledWidth, scaledHeight);
    //image(bumpImage, mapX(center5.x), mapY(center5.y), scaledWidth, scaledHeight);
    //image(bumpImage, mapX(center6.x), mapY(center6.y), scaledWidth, scaledHeight);
  ...
  if (ffNum == FFNUM.FIVE) {
    ...
    // Define the range of x values to draw the polynomial function
    float startX = -0.07;
    float endX = 0.055;
    float step = 0.001; // Step size for x values
    
    // Draw the polynomial function
    for (float x = startX; x <= endX; x += step) {
      float y = calculatePolynomial(x); // Calculate y value for the current x
      float screenX = mapX(x); // Map x value to screen coordinates
      float screenY = mapY(y); // Map y value to screen coordinates
      point(screenX, screenY); // Draw a point representing the function
    }
    
        for (float x = startX; x <= endX; x += step) {
      float y = calculatePolynomial2(x); // Calculate y value for the current x
      float screenX = mapX(x); // Map x value to screen coordinates
      float screenY = mapY(y); // Map y value to screen coordinates
      point(screenX, screenY); // Draw a point representing the function
    }*/
...
/* Large bumps  **************************************************************************************************/      
    ...
    println(posEE);
    //ellipse(posEE.x+100, posEE.y*400, 10, 10);
...
/* PYRAMID GRID  **************************************************************************************************/      
    ...
    // Define the range of x and y coordinates for the grid
        ...
        /*// Example point position (center of the square-based pyramid)
        PVector center = new PVector(0.0, 0.05); // Example point position

        // Calculate the direction vector from the center to the end-effector
        PVector direction = PVector.sub(posEE, center);

        // Define the size of the square-based pyramid (half of the side length)
        float halfSideLength = 0.01f;

        // Scale factor for force
        float forceScale = 8000.0f;

        // Initialize force vector
        PVector force = new PVector();

        // Check if the end-effector is inside the square-based pyramid
        if (Math.abs(direction.x) <= halfSideLength && Math.abs(direction.y) <= halfSideLength) {
            println("INSIDE");
            // Inside the square-based pyramid
            fEE.set(direction.x * forceScale, direction.y * forceScale);
        }*/
...
//CONTOUR FOLLOWING*********************************************************************************************************************
 ...
 /* // Calculate y-value of the polynomial function for the current x-position of the end-effector
      float yOnCurve = calculatePolynomial(posEE.x);
      float yOnCurve2 = calculatePolynomial2(posEE.x);
      //println(yOnCurve, posEE.y);

      // Calculate distance between end-effector's y-position and y-value of the polynomial function
      double distance = Math.abs(posEE.y - yOnCurve);
      double distance2 = Math.abs(posEE.y - yOnCurve2);
      //println(distance);
      threshold = 0.007;

      // Check if end-effector is within threshold distance from the polynomial function
      if (distance <= threshold || distance2 <= threshold) {
      //if (distance <= threshold) {
        println("CLOSE");
        // Apply force pushing end-effector away from the polynomial function
        // For example, you can apply a force opposite to the gradient of the polynomial function
        //************float slope = calculatePolynomialDerivative(posEE.x);
        //************PVector forceDirection = new PVector(1, -slope).normalize(); // Direction opposite to the gradient
        //************float forceMagnitude = 10.0; // Adjust force magnitude as needed
        //************PVector force = forceDirection.mult(forceMagnitude);
    ...
    if(posEE.y > 0.0283 && posEE.y <= 0.0301){
     ...
     /* if (posEE.y < y && lastPosEE.y >= y && abs(posEE.y - y) > threshold) {
    // Apply a force pushing against the end-effector in the upward direction
    fEE.add(0, 50); // Adjust the force direction as needed
  }
  
  // Check if the end-effector crosses the line from below
  if (posEE.y > y && lastPosEE.y <= y && abs(posEE.y - y) > threshold) {
    // Apply a force pushing against the end-effector in the downward direction
    fEE.add(0, -50); // Adjust the force direction as needed
  }*/
  
  // Update lastPosEE for the next frame
  lastPosEE = posEE.copy();
